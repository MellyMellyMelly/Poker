...
namespace HandLanding.Functions
{
    ...
    {
        public static Object[] PlayPoker(int total, Deck deck, int[] specificHand, int[] specificBoard)
        {
            // total represents the number of players in the game
            int[] everything = PreGame.PreDeal(deck, total, specificHand, specificBoard);
            everything = PreGame.SortAllNumbers(everything, total);
            return DetermineWinner(PreGame.CreatePlayers(everything, deck, specificBoard, total));
